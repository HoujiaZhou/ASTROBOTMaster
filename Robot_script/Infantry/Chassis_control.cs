...
public class Chassis_control : MonoBehaviourPun
{
    ...
    void FixedUpdate()
    {
        ...
        if (chassis_Data)
        {
            ...
            movement.z =
                -Vy_Speed * (float)Math.Sin(Angle_gimbal) + Vx_Speed * (float)Math.Cos(Angle_gimbal); //朝摄像机方向运动
            ...
            else if (!Isgroud)
            {
                ...
                if (Isgroud_time >= 5)
                {
                    ...
                    movebody_.position = new Vector3(movebody_.position.x, movebody_.position.y + 10,
                        movebody_.position.z);
                    Isgroud_time = 0;
                }
            }
        }
